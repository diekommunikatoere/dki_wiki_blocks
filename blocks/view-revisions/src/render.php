...
<?php
	...
	// Get revisions
	$revisions = get_revisions($postID);

	// Get my revisions
	$myRevisions = get_my_revisions($postID, $user->ID);

	function get_revisions($postID) {
		// Get posts with post_type "docs" and comment_count equal to the post ID
		// These are the revisions of the post
	...
	function get_my_revisions($postID, $userID) {
		// Get posts with post_type "docs" and comment_count equal to the post ID
		// These are the revisions of the post
		// To make sure that the post is a revision, we check if "post_mime_type" is either "pending-revision" or "draft-revision"
...
<!-- Version for admins and moderators -->
<?php 
	if(in_array('administrator', $userRoles) || in_array('um_moderator', $userRoles)){
		if(count($revisions) > 0 ){ ?>
			<div class="revisions">
				<h4>Alle offenen Änderungsvorschläge</h4>
	...
	<?php } 
	} else if(in_array('um_team', $userRoles)){ ?>
		<!-- Version for team members -->
		<?php if(count($myRevisions) > 0 ){ ?>
			<div class="revisions">
				<h4>Meine offenen Änderungsvorschläge</h4>
				<div class="revision-list">
					<table>
						<thead>
							<tr>
