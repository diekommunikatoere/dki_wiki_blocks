...
document.onreadystatechange = function () {
  if (document.readyState === "complete") {
    ...
    document.getElementById("navbarDropdown").addEventListener("click", handleDropdownToggle.bind(null, dropdown));

    // Close the dropdown if the user clicks outside of it
    window.onclick = function (event) {
      // Close the dropdown menu if the user clicks outside of it
      if (event.target.id !== dropdown.id && dropdown.attributes["aria-expanded"].value === "true") {
    ...
    document.onkeydown = function (event) {
      if (event.key === "Escape" && dropdown.attributes["aria-expanded"].value === "true") {
        handleCloseDropdown(dropdown);
      }
    };
  }
